...
/** A constructed semantic token that contains no delta encoding as of right now. This token can be
  * encoded by its source position later to delta encoding and ordered appropriately. This is
  * largely because I am too stupid to figure out how to do everything in a single pass. Perhaps
  * someone smarter can come up with something that is a less expensive solution, preferrably only
  * one pass.
  *
  * @param length
  *   The length of the token.
  * @param tokenType
  *   The type of the token (use the semantic tokens service to retrieve this value).
  * @param tokenModifiers
  *   Ditto.
  * @param line
  *   The source position of the line of the input text.
  * @param column
  *   Ditto.
  */
